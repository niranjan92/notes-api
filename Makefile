...
db-start: ## start the database server
	...
	docker run --rm --name postgres -v $(shell pwd)/testdata:/testdata \
		-v $(shell pwd)/testdata/postgres:/tmp/postgresql/data \
		-e POSTGRES_PASSWORD=postgres -e POSTGRES_DB=go_restful -d -p 5433:5432 postgres
...
testdata: ## populate the database with test data
	@echo "APP_DSN = $(APP_DSN)"
	make migrate-reset
	@echo "Populating test data..."
	# @docker exec -it postgres psql "$(APP_DSN)" -f /testdata/testdata.sql 
	@docker exec -it postgres psql "postgres://127.0.0.1:5432/go_restful?sslmode=disable&user=postgres&password=postgres" -f /testdata/testdata.sql 
